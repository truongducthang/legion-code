...
Allow users to paste or drop images and files into a terminal so that CLI agents
(Claude Code, etc.) receive a usable absolute filesystem path — not a basename,
not raw bytes, and not a browser-default text fallback. Resolution covers the
macOS and Linux clipboard flavours that file managers actually publish, and
both Cmd+V paste and drag-and-drop entry points, delivering the result through
xterm's bracketed-paste pipeline so agents recognize it as a paste.
