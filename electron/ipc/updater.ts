...
function isAutoUpdateSupported(): boolean {
  ...
  // Without it, electron-updater would throw "Update info is not provided" on
  // every check; treat the build as unsupported instead.
  const resourcesPath = process.resourcesPath;
  if (!resourcesPath || !existsSync(path.join(resourcesPath, 'app-update.yml'))) return false;
